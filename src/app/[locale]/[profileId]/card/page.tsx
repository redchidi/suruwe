...
interface Props {
  params: { locale: string; profileId: string };
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${profileData.name} on Suruwe`;
  const description = `${measurementCount} measurements. ${photoCount || 0} photos. Ready to work with any tailor.`;
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${profileData.name}'s Suruwe card`,
        },
      ],
    },
    twitter: {
...
export default function CardPage({ params }: Props) {
  return <CardClient params={params} />;
}
