...
export default function CardClient({ params }: { params: { profileId: string } }) {
  ...
  const loadProfile = async () => {
    ...
    const { data: orderData } = await supabase
      ...
      .eq('profile_id', p.id)
      .eq('status', 'completed')
      .not('completed_photo_url', 'is', null)
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `${profile?.name} on Suruwe`,
          text: `Check out my Suruwe card`,
          url,
        });
      } catch {}
    } else {
  ...
  const handleWhatsApp = () => {
    const url = window.location.href;
    const text = `My measurements and fit profile are on Suruwe. Pin this link so you always have it: ${url}`;
  ...
  if (notFound || !profile) {
    return (
      <div className="not-found">
        ...
        <p>This Suruwe card does not exist or may have moved.</p>
  ...
  return (
    <div className="app-shell" style={{ paddingBottom: 60 }}>
            ...
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              {measurementCount > 0 && (
                <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                  <span style={{ fontWeight: 600, color: 'var(--text)', marginRight: 4 }}>
                    {measurementCount}
                  </span>
                  measurements
                </div>
              ...
              {orders.length > 0 && (
                <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                  ...
                  completed {orders.length === 1 ? 'piece' : 'pieces'}
                </div>
              )}
              {tailors.length > 0 && (
                <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                  ...
                  {tailors.length === 1 ? 'tailor' : 'tailors'}
                </div>
              )}
            </div>
          </div>
      ...
      {orders.length > 0 && (
        <div className="section">
          <div className="section-title">Completed Pieces</div>
          <div
            style={{
              display: 'grid',
      ...
      <div className="tailor-footer" style={{ marginTop: 40 }}>
        <div className="logo">
          <a href="https://suruwe.vercel.app" style={{ color: 'inherit', textDecoration: 'none' }}>
            Suruwe
          </a>
        </div>
        <p>
