...
export async function POST(request: NextRequest) {
  ...
  try {
    const { prompt, generationCount } = await request.json();
    ...
    // Wrap the user's prompt with clothing-specific context
    const enhancedPrompt = `Fashion design reference image: ${prompt.trim()}. Show the garment on a clean background, full view, suitable as a reference image for a tailor. Professional fashion illustration style.`;
